...
Clinical data are data collected from patients during healthcare delivery, clinical trials, and other medical research. Clinical data are unique because they are heterogeneous, with many different sources and structures. Clinical data include patient demographics, medical records, images, clinician notes, case report forms, and many other types of data. These data can generally be split into two categories: structured data, which are stored in tabular form, and unstructured data, which are typically stored in text files and do not have a specific format. 
