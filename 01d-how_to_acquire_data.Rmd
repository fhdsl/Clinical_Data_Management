...
When proposing new clinical research, a researcher may plan to collect new clinical data or have access to clinical data that they have collected, that a collaborator has collected, or that their institution has access to. However, it is becoming increasingly common to do research on existing clinical data. For these studies, a major question is how to acquire these data. 

When evaluating a source of clinical data, it is useful to understand why the data were collected, who is included in the data, what variables are available, and how the data can be accessed and any associated applications or costs. 
...
The CRDC is a data commons run by the National Cancer Institute (NCI) to accelerate cancer research. The CRDC is really a collection of seven different data commons, as well as a cloud resource and other computing infrastructure. The seven data commons included in the CRDC are:
...
The GDC includes genomic data that are donated from many NIH-funded cancer studies and external studies. A partial list of data sources can be found [here](https://gdc.cancer.gov/about-data/data-sources).
...
The GDC data portal allows researchers to download data or to analyze data using the GDC platform. Data within the GDC are classified as either open access or controlled access. 
...
A major limitation of most registries and EHR platforms is a lack of detailed tumor molecular data. Clinico-genomic data platforms link clinical data with tumor sequencing data. Both clinical data and  sequencing data are collected as part of the healthcare process, but a linked database can be used for deeper research about genomic structures of and treatment for a variety of cancer types.
...
Project GENIE is a publicly accessible cancer-focused clinicl-genomic data resource that combines cancer genomic sequencing data and clinical outcomes, funded by the American Association for Cancer Research (AACR). A major goal of Project GENIE is to collect a large repository of sequencing and clinical cancer data for precision oncology research. Precision medicine customizes a patient's treatment based on their individual genomic, behavioral, and environmental characteristics, and requires a large amount of data. Project GENIE includes clinical and cancer genome sequencing data from most cancer patients treated at twenty international cancer centers. Through partnerships with [Sage Bionetworks](https://sagebionetworks.org) and [cBioPortal](https://www.cbioportal.org), the registry aggregates and harmonizes this data, and provides it freely for research purposes. Project GENIE also has a research collaboration with the Biopharma Collaborative (BPC), a coalition of ten biopharmaceutical companies, in order to gather additional clinical data to the existing Project GENIE data. 
